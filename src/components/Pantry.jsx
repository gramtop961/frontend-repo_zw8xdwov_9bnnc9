...
export default function Pantry({ refreshKey }) {
      ...
      <ul className="divide-y">
        {items.map((t, i) => (
          <li key={t.id || i} className="py-2 flex items-start justify-between">
            ...
            {t.category && <span className="text-xs rounded px-2 py-1 bg-emerald-100 text-emerald-700">{t.category}</span>}
          </li>
        ))}
        {(!items || items.length === 0) && !loading && !error && (
          <li className="py-2 text-sm text-gray-500">No pantry items yet.</li>
        )}
      </ul>
    </div>
  )
