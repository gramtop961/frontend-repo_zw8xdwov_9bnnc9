...
export default function Bills({ refreshKey }) {
      ...
      <ul className="divide-y">
        {items.map((t, i) => (
          <li key={t.id || i} className="py-2 flex items-start justify-between">
            <div>
              <div className="font-medium text-gray-900">{t.name || 'Bill'}</div>
              <div className="text-xs text-gray-500">Due day: {t.due_day || 1}</div>
            </div>
            <span className="text-sm font-semibold text-amber-700">${t.amount?.toFixed?.(2) || Number(t.amount || 0).toFixed(2)}</span>
          </li>
        ))}
